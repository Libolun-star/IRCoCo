...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config, tuning_mode=None, clone_rl_head=False):
        ...
        if self.tuning_mode in ['rl'] and self.clone_rl_head:
            # Optional: have a seperate linear layer for RL training (with synthetic samples)
            self.rl_head = nn.Linear(config.n_embd, config.vocab_size, bias=False)
    ...
    def forward(
        ...
        if rewards is not None:
            if self.clone_rl_head:
                rl_logits = self.rl_head(hidden_states)
                # pass
            else:
                rl_logits = lm_logits

            # log_prob_fct = LogSoftmax(dim=-1)  # 对Softmax的结果进行log
            ...
            shift_logits = rl_logits[..., :-1, :].contiguous()
            shift_labels = labels[..., 1:].contiguous()
            shift_rewards = rewards[..., 1:].contiguous()

            log_prob_fct = LogSoftmax(dim=-1)  # 对Softmax的结果进行log
            probs = log_prob_fct(shift_logits)[shift_labels != -100].unsqueeze(0)

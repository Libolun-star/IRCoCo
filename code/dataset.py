...
class finetuneDataset(Dataset):
    def __init__(self, tokenizer, args, logger, file_type='train', block_size=1024):
        ...
        if os.path.exists(cached_file) and not args.overwrite_cache:
            ...
            if file_type == 'train':
                logger.warning("Creating features from dataset file at %s", datafile)  # 为数据文件创建特征
            with open(datafile) as f:
                data = f.readlines()
            ...
            input_ids = []
            for idx,x in enumerate(data):
                x = x.strip()  # 删除字符串开头和结尾的空格
                if x.startswith("<s>") and x.endswith("</s>"):
                    pass
                else:
                    x = "<s> " + x + " </s>"
                try:
                    input_ids.extend(tokenizer.encode(x))  # 将数据编码为数字,使用tokenizer.encode()
                except Exception:
                    pass
                if idx % (length//10) == 0:
                    percent = idx / (length//10) * 10
            ...
            if file_type == 'train':
                ...
                logger.warning("Saving features into cached file %s", cached_file)
            with open(cached_file, 'wb') as handle:
                pickle.dump(self.inputs, handle, protocol=pickle.HIGHEST_PROTOCOL)  # 将对象序列化存入cached_file中
